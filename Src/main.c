...
static QueueHandle_t xQueueModelOutput =
    NULL; // Canal de datos del modelo hacia motores, TX y LCD
static QueueHandle_t xQueueRawCmd =
    NULL; // Canal para comandos sin procesar desde la interrupción
static QueueHandle_t xQueueValidCmd =
    NULL; // Canal para comandos validados hacia el modelo
static EventGroupHandle_t xEventGroup =
    NULL; // Grupo de eventos para sincronización

#define EVT_BIT_NEW_CMD (1 << 0)

// Aquí definimos los contadores de pulsos de los encoders compartidos con las
// interrupciones
...
int main(void) {
  ...
  // Aquí configuramos las prioridades de interrupción en el NVIC compatibles
  // con FreeRTOS
  ...
  // Aquí creamos las tareas con prioridades asignadas por Rate Monotonic
  // Scheduling (RMS)
...
// Aquí implementamos la tarea que procesa el paso del modelo físico del
// vehículo
static void Task_Model(void *pv) {
  (void)pv;
  ...
  for (;;) {
    ...
    EventBits_t bits =
        xEventGroupWaitBits(xEventGroup, EVT_BIT_NEW_CMD, pdTRUE, pdFALSE, 0);
    ...
    // Aqui determinamos los valores de entrada para el acelerador y el freno
    // del modelo
    if (!(GPIOA->IDR & (1UL << 4U)) ||
        (cmd.remote_control && cmd.remote_brake)) {
      // Freno activo: aplicamos el torque máximo de frenado y ponemos el
      // acelerador a cero
      EngTrModel_U.BrakeTorque = 150000.0;
      EngTrModel_U.Throttle = 0.0;
      adcControl = 0;
    } else {
      // Freno inactivo: seleccionamos la entrada del acelerador (potenciómetro
      // o control remoto)
    ...
    // Aquí limitamos los estados internos del modelo para prevenir
    ...
    // Aquí estructuramos los datos calculados para compartirlos con otras
    // tareas
    ...
    // Aquí enviamos la información actualizando la cola con el dato más
    // reciente
...
// Aquí implementamos la tarea que transmite periódicamente la telemetría a la
// ESP32
static void Task_CommTx(void *pv) {
  (void)pv;

  TickType_t xLastWakeTime = xTaskGetTickCount();
  // Definimos un intervalo de 500 ms para evitar saturar el canal de
  // comunicación
  ...
  for (;;) {
    // Consultamos el dato de la cola sin consumirlo para permitir el acceso a
    // otras tareas
...
// Aquí implementamos la tarea que recibe y valida los comandos de control
// remoto
static void Task_CommRx(void *pv) {
  (void)pv;
  ...
  for (;;) {
    ...
    if (xQueueReceive(xQueueRawCmd, &rawCmd, 0) == pdTRUE) {
      // Aquí validamos los rangos permitidos para el estado y el nivel de
      // aceleración
      ...
      // Aquí activamos el bit de evento para notificar al modelo la presencia
      // de un comando
...
// Aquí implementamos la tarea del lazo de control PID para la velocidad de los
// motores
static void Task_MotorControl(void *pv) {
  (void)pv;
  ...
  for (;;) {
    // Aquí leemos y reiniciamos los contadores de encoders protegiendo la zona
    // crítica
    taskENTER_CRITICAL();
    for (int i = 0; i < 4; i++) {
    ...
    if (setpointRPM == 0) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        controlPWM[i] = feedforward + (int32_t)(Kp * (float)error[i]) +
    ...
    for (int i = 0; i < 4; i++) {
      if (controlPWM[i] < 0)
        controlPWM[i] = 0;
      if (controlPWM[i] > 1000)
        controlPWM[i] = 1000;
    }

    // Aquí actualizamos los registros de comparación del TIM3 con el ciclo
    // calculado
    ...
    // Los mensajes de depuración por USART2 permanecen inactivos para evitar
...
void EXTI9_5_IRQHandler(void) {
  ...
  if (EXTI->PR & (1UL << 8U)) {
    for (volatile int i = 0; i < 160; i++)
      ; // Retardo simple para mitigar rebotes mecánicos a 64 MHz
    if (GPIOA->IDR & (1UL << 8U)) {
      s_encoderPulses[0]++;
    }
    // Aquí limpiamos la bandera de interrupción en el registro EXTI escribiendo
    // un uno lógico
    EXTI->PR = (1UL << 8U);
  }
    ...
    if (rx_c == '\n' || rx_idx >= 31) {
      ...
      if (rx_buf[0] == 'C' && rx_buf[1] == ':') {
        ...
        if (rx_buf[idx] == ',') {
          ...
          //  colocamos el comando en la cola de recepción desde la interrupción
          CommandData_t cmd;
          cmd.remote_control = (uint8_t)st;
          cmd.remote_throttle = (uint16_t)acc;
          cmd.remote_brake = 0;

          // Aqui parseamos el freno
          if (rx_buf[idx] == ',') {
            idx++;
            int brk = 0;
            while (rx_buf[idx] >= '0' && rx_buf[idx] <= '9') {
          ...
          xQueueOverwriteFromISR(xQueueRawCmd, &cmd, &xHigherPriorityTaskWoken);
