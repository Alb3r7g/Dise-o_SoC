...
int main(void)
{
    ...
    if(contador >= 500)
    {
        ...
        USER_USART2_SendString(" THR:");

        USER_IntToStr((uint32_t)EngTrModel_U.Throttle,buffer);

        USER_USART2_SendString(buffer);

        USER_USART2_SendString(" ERR: ");

        USER_IntToStr((uint32_t)(error >= 0 ? error : -error), buffer);

        USER_USART2_SendString(buffer);

        USER_USART2_SendString(" RPM_Model: ");

        USER_IntToStr(rpmModelo, buffer);
